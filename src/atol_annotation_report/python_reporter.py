...
# higher-level functions for mapping

def map_metadata(metadata_arg, all_metadata):
    ''' parse metadata input '''
    if metadata_arg is not None:
        all_metadata["metadata_input_provided"] = True
        logger.info("Parsing metadata")
        with open(metadata_arg, "rt") as f:
...
def map_agat(agat_arg, all_agat_stats, gnl_stats):
    ''' parse AGAT yaml and map to new field names '''
    if agat_arg is not None:
        ...
        gnl_stats.update(key_agat_stats)
    else:
...
def map_busco(busco_arg, all_busco_stats, gnl_stats):
    ...
    if busco_arg is not None:
        ...
        gnl_stats.update(key_busco_stats)
    else:
...
def map_omark(omark_arg, all_omark_stats, gnl_stats):
    ...
    if omark_arg is not None:
        ...
        gnl_stats.update(key_omark_stats)
    else:
...
def main():
    ...
    # mapping input files to output dictionaries
    processed_metadata = map_metadata(metadata_arg=args.metadata_file, all_metadata={})
    processed_agat = map_agat(agat_arg=args.agat_file, all_agat_stats={}, gnl_stats=stats_for_gnl)
    processed_busco = map_busco(busco_arg=args.busco_file, all_busco_stats={}, gnl_stats=stats_for_gnl)
    processed_omark = map_omark(omark_arg=args.omark_file, all_omark_stats={}, gnl_stats=stats_for_gnl)
    processed_annooddities = map_annooddities(oddity_arg=args.annooddities_file, all_oddities={})

    logger.info("Combining statistics and writing to JSON")

    combined_stats = (
        processed_metadata | processed_agat | processed_busco | processed_omark | processed_annooddities
    )
