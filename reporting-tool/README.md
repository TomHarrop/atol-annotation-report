...
The [atol-reporter.py](/reporting-tool/atol_reporter.py) script is an adaptation of the above script. It generates the same report from the same typst template file, and simultaneously generates an atol_report JSON file for use in the AToL genome note lite pipeline.

## Input arguments

[atol-reporter.py](/reporting-tool/atol_reporter.py) takes as input:
...
[atol-reporter.py](/reporting-tool/atol_reporter.py) generates:
 - a PDF report listing the provided metadata and statistics generated in BUSCO and AGAT analyses in human-readable format
 - a JSON report with the same key-value pairs used to generate the PDF above (saved to `"reporting-tool/dev/json_report.json"` )
 - a JSON report containing a small subset of annotation metrics used in the AToL genome-note-lite pipeline (saved to `"reporting-tool/dev/atol_report.json"`)
