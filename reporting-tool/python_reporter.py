...
argument_parser = argparse.ArgumentParser(description="This tool generates a JSON and PDF report of annotation metadata and metric vaules from BUSCO, OMArk, AGAT, and AnnoOddities evaulations for the purposes of QA")
argument_parser.add_argument("-m", "--metadata_file", nargs='?', type=Path, help="a JSON file containing metadata according to the Annotation Metadata Schema")
argument_parser.add_argument("-a", "--agat_file", nargs='?', type=Path, help="a YAML file generated as output from an AGAT analysis on your annotation file")
argument_parser.add_argument("-b", "--busco_file", nargs='?', type=Path, help="a JSON file generated as output from a BUSCO analysis on your annotation file")
argument_parser.add_argument("-om", "--omark_file", nargs='?', type=Path, help="a JSON file generated as output from an OMArk analysis on your annotation file")
argument_parser.add_argument("-ao", "--annooddities_file", nargs='?', type=Path, help="a TXT file summarising any oddities found in the AnnoOddity analysis of your annotation file")
argument_parser.add_argument("-o", "--output_file", type=Path, help="your desired report file address for the output PDF report")
...
combined_stats = all_metadata | agat_output | busco_output | omark_output | oddity_output
...
print("AToL Annotation Report Tool completed. Report available as PDF (" + str(output_pdf) + ") and JSON (" + str(json_full_report) + ")")